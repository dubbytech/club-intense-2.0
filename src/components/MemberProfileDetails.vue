        ...
        <v-container>
                ...
                <v-col cols="12" md="4">
                    <v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" transition="scale-transition" offset-y max-width="290px" min-width="290px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-text-field v-model="dob" label="Date of birth" hint="MM/DD/YYYY format" persistent-hint v-bind="attrs" required v-on="on"></v-text-field>
                        </template>
            ...
            <v-row>
                <v-col cols="12" md="4">
                    ...
                    <v-btn color="error" class="mr-4" @click="deleteProfile">
...
export default {
    data: () => ({
        ...
        zipCodeRules: [
            ...
            v =>
                v.length <= 5 ||
                "Zip code must not be greater than 5 characters"
        ],
        titleRules: [
            v => !!v || "Title is required",
            v =>
                v.length <= 50 || "Title must not be greater than 50 characters"
        ],
        miRules: [
        ...
        addressRules1: [
            ...
            v =>
                v.length <= 50 ||
                "Address1 must not be greater than 50 characters"
        ],
        cityRules: [
        ...
        cellPhoneRules: [
            ...
            v =>
                v.length <= 11 ||
                "Cell Phone must not be greater than 11 characters"
        ],
    ...
    methods: {
        ...
        getHomeTowns() {
            HTTP.get("/api/HomeTown/")
                .then(response =>
                    this.populateHomeTowns(response.data.results.data)
                )
                .catch(() => this.getFailed());
        },
